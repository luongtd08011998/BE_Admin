...
| Method | Endpoint | Auth | Mô tả |
|--------|----------|------|-------|
| POST | /external/qr-payment/generate | 🔑 | Tạo mã VietQR |
...
└── vnpt/           (giữ nguyên)
```
*(Ghi chú: Giữ Admin Invoice và Admin Road bên trong `qlkh/` để tương thích cấu trúc quét Multiple Data Sources của Spring JPA, tránh lỗi query nhầm sang database `primary`)*
